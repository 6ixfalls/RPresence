...
function checkForRoGold() {
    return new Promise((resolve, reject) => {
        var img = new Image();
        img.src = "chrome-extension://mafcicncghogpdpaieifglifaagndbni/images/oldest.png";
...
var gameInstanceId = getQueryVariable("gameInstanceID");
var gameName = getQueryVariable("gameName");

if (!placeID || !gameInstanceId || !gameName) {
    window.location.href = "https://github.com/6ixfalls/RPresence/";
}

(async () => {
    document.getElementById("gameThumbnail").addEventListener("load", (e) => {
    ...
    document.getElementById("joinGame").addEventListener("click", (e) => {
        window.open(`https://roblox.com/discover#/rg-join/${placeID}/${gameInstanceId}`);
    });

    if (isMobile()) {
        window.location = `robloxmobile://placeID=${placeID}&gameInstanceId=${gameInstanceId}`;
    } else {
